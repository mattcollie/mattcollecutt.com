<script lang="ts">
	...
	let { mobile = false, open = $bindable(false) }: { mobile?: boolean; open?: boolean } = $props();
...
{#snippet sidebarContent()}
	<Banner border={true}>
		<div class="w-full h-full flex flex-row gap-4 items-center pl-6">
			{#if mobile}
				<button onclick={closeSheet} aria-label="Close navigation">
	...
	<SidebarLinkGroup>
		<SidebarLink href="/" onclick={mobile ? closeSheet : undefined}>
	...
	<SidebarLinkGroup title="Projects">
		<SidebarLink href="https://trivino.xyz" target="_blank" external={true} onclick={mobile ? closeSheet : undefined}>
		...
		<SidebarLink href="https://nookly.co.nz" target="_blank" external={true} onclick={mobile ? closeSheet : undefined}>
	...
	<SidebarLinkGroup title="Socials">
		<SidebarLink href="https://github.com/mattcollie" target="_blank" external={true} onclick={mobile ? closeSheet : undefined}>
			<img height="12" width="12" src="/images/github.svg" alt="Github icon" loading="lazy" />
			<span class="text-xs font-semibold">Github</span>
		</SidebarLink>
		<SidebarLink href="https://www.linkedin.com/in/matt-collecutt" target="_blank" external={true} onclick={mobile ? closeSheet : undefined}>
			<img height="12" width="12" src="/images/linkedin.svg" alt="Linkedin icon" loading="lazy" />
			<span class="text-xs font-semibold">Linkedin</span>
		</SidebarLink>
		<SidebarLink href="https://www.instagram.com/matthewcollecutt" target="_blank" external={true} onclick={mobile ? closeSheet : undefined}>
			<img height="12" width="12" src="/images/instagram.svg" alt="Instagram icon" loading="lazy" />
			<span class="text-xs font-semibold">Instagram</span>
		</SidebarLink>
	</SidebarLinkGroup>

	<div class="flex-1"></div>
	<div class="w-full !min-h-[50px] border-t border-t-neutral-800 p-2"></div>
{/snippet}

{#if !mobile}
	<!-- Desktop sidebar -->
	<nav class="hidden lg:flex fixed z-30 h-full max-h-screen min-h-screen w-56 2xl:w-80 flex-col overflow-y-auto border-r border-r-stroke bg-secondary pb-10">
		{@render sidebarContent()}
	</nav>
{:else}
	<!-- Mobile sidebar (Sheet) -->
	<Sheet bind:open>
		<SheetContent {open} side="left" onclose={closeSheet}>
			<nav class="flex flex-col h-full overflow-y-auto bg-secondary pb-10">
				{@render sidebarContent()}
			</nav>
		</SheetContent>
	</Sheet>
